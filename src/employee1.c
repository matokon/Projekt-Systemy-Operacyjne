#include <stdio.h>
#include <unistd.h>
#include <string.h>
#include <stdlib.h>
#include <signal.h>
#include <time.h>
#include <errno.h>
#include <sys/msg.h>
#include "simulation.h"
#include "cablecar.h"
#include "ipc.h"
#include "platform_queue.h"

static volatile sig_atomic_t g_stopped = 0;
static volatile sig_atomic_t g_await_ack = 0;
static volatile sig_atomic_t g_initiator = 0;
static volatile sig_atomic_t g_sent_ack = 0;
static volatile sig_atomic_t g_other_pid = 0;

/* Obsluga SIGUSR1: zatrzymuje pracownika 1 i resetuje flagi. */
static void on_sigusr1(int sig) {
    (void)sig;
    g_stopped = 1;
    g_initiator = 0;
    g_sent_ack = 0;
    printf(CLR_CYAN"    Pracownik1 %d: STOP (signal1)\n" RESET, getpid());
}

/* Obsluga SIGUSR2: wznawia prace, ewentualnie wysyla potwierdzenie do partnera. */
static void on_sigusr2(int sig) {
    (void)sig;
    g_stopped = 0;
    printf(CLR_CYAN"    Pracownik1 %d: WZNOWIENIE (signal2)\n" RESET, getpid());
    if (g_await_ack) {
        g_await_ack = 0;
        return;
    }
    if (!g_sent_ack && g_other_pid > 0) {
        g_sent_ack = 1;
        kill((pid_t)g_other_pid, SIGUSR2);
    }
}

/* Rejestruje obsluge sygnalow i zapisuje PID pracownika w pamieci cablecar. */
static void init_stop_signals(cablecar_t *cablecar, int sem_shm) {
    signal(SIGUSR1, on_sigusr1);
    signal(SIGUSR2, on_sigusr2);
    ipc_sem_wait(sem_shm);
    cablecar->emp1_pid = getpid();
    ipc_sem_post(sem_shm);
}

/* Synchronizuje z drugim pracownikiem przez semafor gotowosci i odczytuje jego PID. */
static void wait_for_other_pid(cablecar_t *cablecar, int sem_shm) {
    int sem_ready = ipc_get_sem_from_env(IPC_ENV_SEM_EMP_READY);
    /* sygnalizuj gotowosc swojego PID */
    ipc_sem_post(sem_ready);
    /* czekaj na gotowosc drugiego pracownika */
    ipc_sem_wait(sem_ready);
    ipc_sem_wait(sem_shm);
    g_other_pid = cablecar->emp2_pid;
    ipc_sem_post(sem_shm);
}

/* Jednorazowo zatrzymuje prace na ok. 9 s (stop_once), inicjujac sygnal do partnera. */
static void maybe_stop_once(int stop_once, int *stop_once_done,
                            time_t start_time, time_t *stop_since) {
    if (!stop_once || *stop_once_done || g_stopped) return;
    if (time(NULL) - start_time < 5) return;
    g_initiator = 1;
    g_stopped = 1;
    g_sent_ack = 0;
    *stop_since = time(NULL);
    kill((pid_t)g_other_pid, SIGUSR1);
    *stop_once_done = 1;
}

/* Sprawdza czy pora wznowic prace po stop_once i wysyla SIGUSR2 do partnera. */
static void maybe_resume(time_t stop_since) {
    if (!g_stopped || !g_initiator || g_await_ack) return;
    if (stop_since <= 0 || time(NULL) - stop_since < 9) return;
    g_await_ack = 1;
    g_sent_ack = 1;
    kill((pid_t)g_other_pid, SIGUSR2);
}

/* Opróżnia kolejkę peronu po komendzie SHUTDOWN, wysyłając PLAT_SHUTDOWN
 * do wszystkich oczekujących turystów (VIP i normalnych), żeby żaden nie
 * zawisł na ipc_recv_platform. */
/* Sciaga zalegle PLAT_REQ z kolejki platformowej i odsyla PLAT_SHUTDOWN. */
static void drain_platform_queue_shutdown(int qid) {
    int guard = -1;
    const char *q = getenv(IPC_ENV_PLATFORM_QID);
    if (q && *q && (int)strtol(q, NULL, 10) == qid) {
        const char *g = getenv(IPC_ENV_SEM_PQ_GUARD);
        if (g && *g) guard = (int)strtol(g, NULL, 10);
    }

    while (1) {
        platform_msg_t msg;
        memset(&msg, 0, sizeof(msg));
        struct msqid_ds ds;
        if (msgctl(qid, IPC_STAT, &ds) == -1) {
            perror("msgctl(IPC_STAT) drain platform");
            break;
        }
        if (ds.msg_qnum == 0) break; /* nic do odbioru, wyjdz */

        ssize_t r = msgrcv(qid, &msg, PLATFORM_MSGSZ, -MT_NORMAL, 0);
        if (r >= 0) {
            if (guard > 0) ipc_sem_post(guard); /* zwolnij slot kredytu kolejki */
            if (msg.kind == PLAT_REQ && msg.pid > 0) {
                platform_send_shutdown(qid, msg.pid);
            }
            continue;
        }
        if (errno == EINTR) continue;
        perror("msgrcv(drain platform)");
        break;
    }
}

/* Glowna petla pracownika 1: obsluga peronu, stop_once, shutdown. */
int main() {
    printf(CLR_CYAN"    Pracownik1 Start: %d" RESET "\n", getpid());

    const char *s1 = getenv(IPC_ENV_PLATFORM_QID);
    if (!s1 || !*s1) {
        fprintf(stderr, "Brak %s w env\n", IPC_ENV_PLATFORM_QID);
        return 1;
    }
    int platform_qid = (int)strtol(s1, NULL, 10);

    const char *s2 = getenv(IPC_ENV_SHM_CABLECAR);
    if (!s2 || !*s2) { 
        fprintf(stderr, "Brak %s w env\n", IPC_ENV_SHM_CABLECAR);
        return 1;
     }
    int shmid = (int)strtol(s2, NULL, 10);
    cablecar_t *cablecar = (cablecar_t*)ipc_attach_shm(shmid);
    int sem_shm = ipc_get_sem_from_env(IPC_ENV_SEM_SHM);
    int sem_chairs = ipc_get_sem_from_env(IPC_ENV_SEM_CHAIRS);

    init_stop_signals(cablecar, sem_shm);

    int stop_once = 0;
    const char *s3 = getenv(IPC_ENV_STOP_ONCE);
    if (s3 && *s3) stop_once = strtol(s3, NULL, 10) != 0;
    int stop_once_done = 0;
    time_t start_time = time(NULL);

    wait_for_other_pid(cablecar, sem_shm);


    enum { MAX_QUEUE = 5000 };
    pid_t bikers[MAX_QUEUE];
    ped_group_t peds[MAX_QUEUE];
    int bikers_n = 0;
    int peds_n = 0;
    int closing = 0;
    time_t stop_since = 0;
    pid_t shutdown_requester = 0;

    for (;;) {
        platform_msg_t req;
        memset(&req, 0, sizeof(req));
        if (ipc_recv_platform(platform_qid, -MT_NORMAL, &req, 0) < 0) break;

        maybe_stop_once(stop_once, &stop_once_done, start_time, &stop_since);
        maybe_resume(stop_since);

        if (req.kind == PLAT_SHUTDOWN) {
            if (!closing) {
                closing = 1;
                platform_flush_shutdown_waiters(platform_qid, bikers, &bikers_n, peds, &peds_n);
            }
            shutdown_requester = req.pid;
            /* wyślij PLAT_SHUTDOWN do wszystkich zaległych żądań zanim wyjdziemy */
            drain_platform_queue_shutdown(platform_qid);
            if (shutdown_requester > 0) {
                platform_msg_t ack;
                memset(&ack, 0, sizeof(ack));
                ack.mtype = (long)shutdown_requester;
                ack.kind = PLAT_SHUTDOWN_ACK;
                ack.pid = getpid();
                ipc_send_platform(platform_qid, &ack);
            }
            printf(CLR_CYAN"    Pracownik1 %d: shutdown final" RESET "\n", getpid());
            break;
        }
        if (req.kind != PLAT_REQ || req.pid <= 0) {
            continue;
        }

        if (!closing && sim_is_closed()) {
            closing = 1;
            printf(CLR_CYAN"    Pracownik1 %d: bramki peronu zamkniete (po Tk)\n" RESET, getpid());
            platform_flush_shutdown_waiters(platform_qid, bikers, &bikers_n, peds, &peds_n);
            platform_send_shutdown(platform_qid, req.pid);
            continue;
        }

        if (closing) {
            platform_send_shutdown(platform_qid, req.pid);
            continue;
        }

        platform_enqueue_request(req.is_biker, req.pid, req.group_size,
                                 bikers, &bikers_n, peds, &peds_n, MAX_QUEUE);
        if (!g_stopped) {
            platform_try_form_groups(platform_qid, cablecar, sem_shm, sem_chairs,
                                     bikers, &bikers_n, peds, &peds_n);
        }
    }

    printf(CLR_CYAN"    Pracownik1 Koniec: %d" RESET "\n", getpid());
    return 0;
}
